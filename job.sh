...
# ============================== 筛选相关测试用例 ==============================
# 定义Defects4J中该缺陷对应的"相关测试用例"文件路径 (可选：trigger_tests, relevant_tests)
SELECTED_TESTS_FILE=${D4J_HOME}/framework/projects/${PID}/relevant_tests/${BID}

# 处理相关测试用例：
# 1. 每行末尾添加"#*"（GZOLTAR识别的测试方法通配符格式）
# 2. 将换行符替换为冒号（GZOLTAR --includes参数要求的分隔符格式）
RELEVANT_TESTS=$(cat ${SELECTED_TESTS_FILE} | sed 's/$/#*/' | sed ':a;N;$!ba;s/\n/:/g')
...
cat ${D4J_HOME}/framework/projects/${PID}/relevant_tests/${BID} | sed -n 's/--- //p' | sort > ${PROJECT_DIR}/d4j_tests_tmp
...
if [[ -n ${DIFF_INFO} ]]; then
	echo -e "${PID}-${BID}:\n${DIFF_INFO}" >> /root/locate_result/exec_info_relevant/${PID}/log
fi
